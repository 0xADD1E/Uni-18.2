...
fn main() -> Result<(),failure::Error> {
    ...
    let mat = re
        ...
        .filter_map(|cap|{trace!("Working on parsing regex match: {:?}", cap);cap.get(0)})
        ...
        .map(|row|row.chars()
            ...
            .map(|c|
            {
                trace!("Parsing individual character (should be 0 or 1): '{}'",c);match c{
                "0" => false,
                "1" => true,
                _ => false
            }})
    ...
    let mat = mat.into_iter().map(|sub|sub.into_iter().map(|entry|match entry{
        true => '1',
        false => '0'
    }).collect::<Vec<char>>()).collect::<Vec<Vec<char>>>();
    let format_str = format!("{:?}", mat).replace("[","{").replace("]","}").replace("'","");
    println!("{}",format_str);
    Ok(())  
}
...
fn compute_symmetric_closure(mat: &Vec<Vec<bool>>)->Vec<Vec<bool>>{
    ...
    for i in 0..new_mat.len(){
        for j in 0..new_mat.len(){
            trace!("Matrix at {},{} was {}, now is {}",i,j,new_mat[i][j], new_mat[i][j]|transpose_mat[i][j]);
